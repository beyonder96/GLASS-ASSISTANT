...
const NoteModal: React.FC<{ isOpen: boolean; onClose: () => void; taskId: string; initialNote: string; onSave: (id: string, note: string) => void }> = ({ isOpen, onClose, taskId, initialNote, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/20 backdrop-blur-sm animate-in fade-in duration-200">
      <GlassCard className="w-full max-w-sm shadow-2xl" padding="p-6">
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            <StickyNote size={20} className="text-indigo-500" />
            <h3 className="font-black text-slate-800 uppercase tracking-widest text-xs">Nota da Tarefa</h3>
          </div>
        ...
        <textarea
          ...
          className="w-full h-32 p-4 rounded-2xl bg-slate-50 border border-slate-100 text-slate-700 font-medium text-sm focus:outline-none focus:ring-2 focus:ring-indigo-100 resize-none mb-4"
        />
        <button
          onClick={() => { onSave(taskId, note); onClose(); }}
          className="w-full py-3 rounded-xl bg-slate-900 text-white font-bold text-xs uppercase tracking-widest shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
        >
...
export const TaskScreen: React.FC<TaskScreenProps> = ({ onBack }) => {
  ...
  const updateTaskNote = (id: string, note: string) => {
    setTasks(prev => prev.map(t => t.id === id ? { ...t, notes: note } : t));
  };
  ...
  return (
    <div className="flex flex-col h-full w-full animate-in slide-in-from-right duration-500">
          ...
          filteredTasks.map((task) => (
            ...
            >
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={() => setSelectedTaskForNote(task.id)}
                  className={`p-2 rounded-lg transition-all active:scale-90 ${task.notes ? 'text-indigo-500 bg-indigo-50 hover:bg-indigo-100' : 'text-slate-400 hover:bg-slate-100'}`}
                >
                  <FileText size={18} />
                </button>
                <button
      ...
      {selectedTaskForNote && (
        <NoteModal
          isOpen={!!selectedTaskForNote}
          onClose={() => setSelectedTaskForNote(null)}
          taskId={selectedTaskForNote}
          initialNote={tasks.find(t => t.id === selectedTaskForNote)?.notes || ''}
